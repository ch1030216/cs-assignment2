...
export default function Home() {
  ...
  const [todos, setTodos] = useState(() => {
    if (typeof window !== "undefined") {
      ...
      return saved ? JSON.parse(saved) : { "2026-05-30": [], "2026-05-31": [] };
    }
    return { "2026-05-30": [], "2026-05-31": [] };
  });
  ...
  const [musicData, setMusicData] = useState(() => {
    if (typeof window !== "undefined") {
      ...
      return saved ? JSON.parse(saved) : { currentTrack: "곡을 선택하세요", isPlaying: false };
    }
    return { currentTrack: "곡을 선택하세요", isPlaying: false };
  });
  ...
  }, [diaryData, todos, deadlines, musicData]);
  ...
  return (
    <div className={`min-h-screen p-6 md:p-10 flex flex-col justify-center items-center font-sans transition-colors duration-300 ${
      ...
      <div className={`w-full max-w-7xl grid grid-cols-1 md:grid-cols-2 gap-6 p-6 rounded-3xl shadow-xl border transition-all duration-300 ${
        ...
        <div className={`p-5 rounded-2xl shadow-sm transition-colors ${darkMode ? "bg-slate-900/90 border border-slate-800" : "bg-white border border-sky-100"}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div className="flex flex-col gap-6">
            <div className={`flex-1 p-5 rounded-2xl border shadow-sm flex flex-col justify-between transition-colors ${darkMode ? "bg-slate-800/50 border-slate-700" : "bg-sky-50/50 border-sky-100/60"}`}>
              <TodoList selectedDate={selectedDate} todos={todos} setTodos={setTodos} darkMode={darkMode} />
            </div>
            <div className={`p-5 rounded-2xl border shadow-sm transition-colors ${darkMode ? "bg-slate-800/50 border-slate-700" : "bg-sky-50/50 border-sky-100/60"}`}>
              <MusicPlayer darkMode={darkMode} selectedDate={selectedDate} musicData={musicData} setMusicData={setMusicData} />
            </div>
          </div>

          <div className="flex flex-col gap-6">
            <div className={`flex-1 p-5 rounded-2xl border shadow-sm flex flex-col gap-3 transition-colors ${darkMode ? "bg-slate-800/50 border-slate-700" : "bg-sky-50/50 border-sky-100/60"}`}>
              ...
              <textarea
                ...
                className={`w-full flex-1 min-h-[150px] p-3 text-xs border rounded-xl focus:outline-none shadow-sm resize-none font-medium transition-colors ${
                  darkMode ? "bg-slate-800 border-slate-700 text-slate-100 focus:border-sky-500 placeholder-slate-500" : "bg-white border-sky-100 text-sky-900 focus:border-sky-400 placeholder-sky-300"
                }`}
              />
            </div>
            <div className={`p-5 rounded-2xl border shadow-sm transition-colors ${darkMode ? "bg-slate-800/50 border-slate-700" : "bg-sky-50/50 border-sky-100/60"}`}>
